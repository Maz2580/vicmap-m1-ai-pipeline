...
class FeatureLayer:
    ...
    def query(self, where_clause: str, out_fields: str = '*', 
        ...
        params = {
            ...
            'f': 'json',
            'resultRecordCount': result_record_count if result_record_count else 10  # Default limit
        }
            
        try:
            response = requests.get(f"{self.url}/query", params=params, timeout=300)  # 5 minutes for large datasets
